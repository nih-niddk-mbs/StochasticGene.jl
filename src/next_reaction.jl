# Julia 1.5
# Next Reaction Stochastic Simulator to compute:
...
struct Greactions
	nstates::Int
	reactions::Vector{GRreaction}
end


function T(reactions::Greactions)


end

function T(reactions)
    nT = n+1
	T = zeros(nT,nT)
	T[1,1] = -gammap[2]
	T[1,2] = gamman[2]
...
struct GMstate <: State
	Gstate::Int
	Mstate::Int
end

struct GRreaction <: Reaction
	Initial::Int
	Final::Int
	Rate::Float64
end

struct GMreaction <: Reaction
	Initial::GMState
	Final::GMState
	Rate::Float64
end

function set_reactions(model::AbstractGMmodel,data)
	reactions = Vector{GMReaction}(undef,total)
	set_reactions!(reactions,model)
end

function set_reactions!(reactions::Vector{GMReaction},model::AbstractGMmodel)
	for m in 1:model.nhist
		for g in 1:model.G
			reactions[i] = GMReaction(g+m,model.r[i])
		end
	end
end

function set_reactions!(reactions::Vector{GMReaction},model::AbstractGRMmodel)
	for m in 1:model.nhist
		for g in 1:model.G
			reactions[i] = GMReaction(g+m,model.r[i])
		end
...
function states(n,nr)


end

function states(n,nr,m)


end

function states(n,nr,ns)


end


function transition_rate_mat(reactions)
	T = zeros(N,N)
	for reaction in reactions
		T[reaction[1]] = reaction[2]
	end
	T
end
...
nstates = (n+1)*base^nr
nreactions = 2*n + 2 + (base-1)*nr

gstate = Vector{Int}(undef,nalleles)

function initialize_propensities(,nalleles)
	af = Array{Float64,1}(undef,nalleles)  # forward
	ab = Array{Float64,1}(undef,nalleles)  # backward
	atranscribe = nu*ones(nalleles).*(state .== 1)    # mRNA creation
	adecay = 0.       # mRNA decay
...
function initialize_times(a)


end

function find_reaction(index)
...
function state_transition!(a,tau,ind,nallele,gammaf,gammab)


end

function next_reaction(tau)
	val,ind = findmin(tau)
	G,m = reactions![ind[1]](a,tau,ind[2],G,m)
	return ind[1], ind[2]
end


function set_reactions()

reactions! = Vector{Function}(undef,4)
reactions![1] = gforward!
reactions![2] = gbackward!
reactions![3] = initiate!
reactions![4] = decay!

return reactions
end

function gforward!(a,tau,nallele,G,m,t)
	tau[1,nallele] = -a1new*log(rand()) + t
	tau[2,nallele] = Inf
	a[1,nallele] = 0.
	a[2,nallele] = gammab[1]
	return m
end

function gbackward!(a,tau,nallele,G,m,t)
	a2new = gammab[1]
	tau[1,nallele] = Inf
	tau[2,nallele] = -a2new*log(rand()) + t
	a[1,nallele] = gammaf[1]
	a[2,nallele] = 0.
	return m
end

function initiate!(a,tau,nallele,G,m,t)
	tau[end,nallele] = -nu*log(rand()) + t
	a[end,nallele] = nu
	m -= 1
	return m
end

function rforward!(a,tau,nallele,G,m,t)

end

function splice!(a,tau,nallele,G,R,m,t)

end

function decay!(a,tau,nallele,G,m,t)
	anew = m*adecay
	tau[end,nallele] = -anew*log(rand()) + t
	a[end,nallele] = anew
	m -= 1
	return m
end
