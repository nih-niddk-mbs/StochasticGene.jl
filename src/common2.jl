...
function likelihoodfn(param::Vector,data::RNAData,model::AbstractGRMmodel)
    r = get_rates(param,model)
    M = make_mat_M(model.components,r)
    steady_state(M,model.G,model.nalleles,data.nRNA)
end
...
#         M = make_mat_M(model.components,r)
#         T = make_mat_T(model.components,r)
#         TA = make_mat_TA(model.components,r)
#         TI = make_mat_TI(model.components,r)
#         modelOFF, modelON = offonPDF(T,TA,TI,data.bins,r,model.G-1,model.R,model.method)
