...
of the numerical method and a Bool for hierarchical models.  However, in biowulf.jl, the numerical method must be a String, i.e. use "Tsit5()" for Tsit5().  This is because when Julia writes
...
function makeswarm(; gene::String="", nchains::Int=2, nthreads=1, swarmfile::String="fit", juliafile::String="fitscript", datatype::String="", dttype=String[], datapath="", cell::String="", datacond="", traceinfo=(1.0, 1.0, -1, 1.0), infolder::String="", resultfolder::String="test", inlabel::String="", label::String="",
    ...
    # Create filedir if it doesn't exist
...
function makeswarm(genes::Vector{String}; nchains::Int=2, nthreads=1, swarmfile::String="fit", batchsize=1000, juliafile::String="fitscript", datatype::String="rna", dttype=String[], datapath="", cell::String="HCT116", datacond="MOCK", traceinfo=(1.0, 1.0, -1, 1.0), infolder::String="HCT116_test", resultfolder::String="HCT116_test", inlabel::String="", label::String="",
    ...
    propcv=0.01, maxtime=60.0, samplesteps::Int=1000000, warmupsteps=0, annealsteps=0, temp=1.0, tempanneal=100.0, temprna=1.0, burst=false, optimize=false, writesamples=false, method="Tsit5()", src="", zeromedian=false, datacol=3, ejectnumber=1, filedir=".", project="")
    # Create filedir if it doesn't exist
    if !isempty(filedir) && !isdir(filedir)
        mkpath(filedir)
    end
...
function makeswarm_models(models::Vector{ModelArgs}; gene="", nchains::Int=2, nthreads=1, swarmfile::String="fit", juliafile::String="fitscript", datatype::String="", dttype=String[], datapath="", cell::String="", datacond="", traceinfo=(1.0, 1.0, -1, 1.0), infolder::String="", resultfolder::String="test",
    ...
    write_swarmfile(joinpath(filedir, sfile), nchains, nthreads, juliafile, datatype, datacond, cell, models, project)
...
function write_fitfile(fitfile, nchains, datatype, dttype, datapath, gene, cell, datacond, traceinfo, infolder, resultfolder, inlabel, label,
    fittedparam, fixedeffects, transitions, G, R, S, insertstep, coupling, grid, root, maxtime, elongationtime, priormean, nalleles, priorcv, onstates,
    decayrate, splicetype, probfn, noisepriors, hierarchical, ratetype, propcv, samplesteps, warmupsteps, annealsteps, temp, tempanneal, temprna, burst, optimize, writesamples, method, src, zeromedian, datacol, ejectnumber, prerun=0.)
    s = '"'
    # Written script must receive Tuple (transitions is tuple of tuples of vectors, or tuple of vectors)
...
"""
    create_modelstring_coupled(G, R, S, insertstep, coupling_field::String)

Model string for coupled runs: base model string plus coupling identifier for script/filename use.
Supports legacy 4-char reciprocal (e.g. "3131") and extended format (e.g. "24,33|33").
"""
function create_modelstring_coupled(G::Tuple, R, S, insertstep, coupling_field::String)
    base = create_modelstring(G, R, S, insertstep)
    isempty(coupling_field) && return base
    return base * "_" * coupling_field
end

"""
    makeswarmcoupled_reciprocal(; coupling_field::String, G::Tuple, R::Tuple, S::Tuple, insertstep::Tuple, ...)

Write swarm and fit files for coupled (reciprocal or extended) tracejoint models.

# Arguments
- `coupling_field::String`: Coupling identifier: 4-char reciprocal (e.g. "3131") or extended (e.g. "24,33|33").
- `G::Tuple`, `R::Tuple`, `S::Tuple`, `insertstep::Tuple`: Per-unit model dimensions.
- `datacond`: Condition string; if empty and coupling_field is set, uses "condition" * coupling_field for tracejoint.
- Other keyword arguments as in `makeswarm`.

Coupling is converted via `make_coupling(coupling_field, G, R)` so extended format is supported.
"""
function makeswarmcoupled_reciprocal(; gene::String="", nchains::Int=2, nthreads=1, swarmfile::String="fit", juliafile::String="fitscript", datatype::String="tracejoint", dttype=String[], datapath="", cell::String="", datacond="", traceinfo=(1.0, 1.0, -1, 1.0), infolder::String="", resultfolder::String="test", inlabel::String="", label::String="",
    fittedparam::Vector=Int[], fixedeffects=tuple(), transitions=([1, 2], [2, 1]), G=(2, 2), R=(0, 0), S=(0, 0), insertstep=(1, 1), coupling_field::String="", TransitionType="", grid=nothing, root=".", elongationtime=6.0, priormean=Float64[], nalleles=1, priorcv=10.0, onstates=Int[], decayrate=-1.0, splicetype="", probfn=prob_Gaussian, noisepriors=[], hierarchical=tuple(), ratetype="median",
    propcv=0.01, maxtime=60.0, samplesteps=Int(1000000), warmupsteps=0, annealsteps=0, temp=1.0, tempanneal=100.0, temprna=1.0, burst=false, optimize=false, writesamples=false, method="Tsit5()", src="", zeromedian=false, datacol=3, ejectnumber=1, project="", prerun=0., sysimage="", filedir=".")
    isempty(coupling_field) && throw(ArgumentError("makeswarmcoupled_reciprocal requires coupling_field (e.g. \"3131\" or \"24,33|33\")"))
    coupling = make_coupling(coupling_field, G, R)
    if isempty(datacond)
        datacond = "condition" * coupling_field
    end
    if !isempty(filedir) && !isdir(filedir)
        mkpath(filedir)
    end
...
"""
    makeswarmcoupled_reciprocal(genes::Vector{String}; coupling_field::String, G::Tuple, ...)

Write swarm and fit files for coupled models for each gene in `genes`.
"""
function makeswarmcoupled_reciprocal(genes::Vector{String}; nchains::Int=2, nthreads=1, swarmfile::String="fit", batchsize=1000, juliafile::String="fitscript", datatype::String="tracejoint", dttype=String[], datapath="", cell::String="HCT116", datacond="", traceinfo=(1.0, 1.0, -1, 1.0), infolder::String="", resultfolder::String="test", inlabel::String="", label::String="",
    fittedparam::Vector=Int[], fixedeffects=tuple(), transitions::Tuple=([1, 2], [2, 1]), G=(2, 2), R=(0, 0), S=(0, 0), insertstep=(1, 1), coupling_field::String="", TransitionType="", grid=nothing, root=".", elongationtime=6.0, priormean=Float64[], nalleles=1, priorcv=10.0, onstates=Int[], decayrate=-1.0, splicetype="", probfn=prob_Gaussian, noisepriors=[], hierarchical=tuple(), ratetype="median",
    propcv=0.01, maxtime=60.0, samplesteps=Int(1000000), warmupsteps=0, annealsteps=0, temp=1.0, tempanneal=100.0, temprna=1.0, burst=false, optimize=false, writesamples=false, method="Tsit5()", src="", zeromedian=false, datacol=3, ejectnumber=1, filedir=".", project="")
    isempty(coupling_field) && throw(ArgumentError("makeswarmcoupled_reciprocal requires coupling_field"))
    if isempty(datacond)
        datacond = "condition" * coupling_field
    end
    if !isempty(filedir) && !isdir(filedir)
...
"""
    create_combined_files(unit1_files::Vector{String}, unit2_files::Vector{String}, outdir::String, coupling_field::String, G::Tuple, R::Tuple; coupling_ranges=nothing, addlabel::Bool=true)

Join per-unit rate files into combined rate files (one per gene). Supports legacy 4-char and extended coupling.
Column order: unit1 rates, unit2 rates, coupling strengths γ in to_connections order.

When `coupling_ranges` is set (a Symbol or tuple of Symbols per connection), default γ values are interior to the
constraint: :activate → 0.1, :inhibit → -0.1, :free → 0.0 (weak coupling). Starting at 0 is on the boundary for both activate and inhibit.
"""
function create_combined_files(unit1_files::Vector{String}, unit2_files::Vector{String}, outdir::String, coupling_field::String, G::Tuple, R::Tuple; coupling_ranges=nothing, addlabel::Bool=true)
    (isempty(unit1_files) || length(unit1_files) != length(unit2_files)) && throw(ArgumentError("unit1_files and unit2_files must be same-length non-empty"))
    coupling = coupling_ranges === nothing ? make_coupling(coupling_field, G, R) : make_coupling(coupling_field, G, R; coupling_ranges=coupling_ranges)
    nconn = isempty(coupling) ? 0 : coupling[5]
    nγ = nconn
    default_γ = nγ > 0 && length(coupling) >= 6 ? default_coupling_prior_means(coupling) : (nγ > 0 ? zeros(nγ) : Float64[])
    mkpath(outdir)
    for (f1, f2) in zip(unit1_files, unit2_files)
        c1, h1 = readdlm(f1, ',', header=true)
        c2, h2 = readdlm(f2, ',', header=true)
        nrow1, ncol1 = size(c1, 1), length(h1)
        nrow2, ncol2 = size(c2, 1), length(h2)
        nrow1 == 0 || nrow2 == 0 && continue
        # Use first data row from each; assume same gene order
        h1_str = String.(vec(h1))
        h2_str = String.(vec(h2))
        if addlabel
            h1_str = h1_str .* "_u1"
            h2_str = h2_str .* "_u2"
        end
        γ_labels = nγ > 0 ? ["γ_$k" for k in 1:nγ] : String[]
        header = vcat(h1_str, h2_str, γ_labels)
        outname = basename(f1)
        outpath = joinpath(outdir, "combined_" * outname)
        endswith(outpath, ".txt") || (outpath = outpath * ".txt")
        f = open(outpath, "w")
        writedlm(f, reshape(header, 1, :), ',')
        for row in 1:min(nrow1, nrow2)
            r1 = vec(c1[row, :])
            r2 = vec(c2[row, :])
            γ_vals = copy(default_γ)
            row_combined = vcat(r1, r2, γ_vals)
            writedlm(f, reshape(row_combined, 1, :), ',')
        end
        close(f)
    end
end
...
"""
    create_combined_files(resultfolder::String, filepattern::String, coupling_field::String, G::Tuple, R::Tuple; outfile::String="")

Create a combined summary of coupled rate files: list all rate files matching pattern and coupling, optionally write a manifest.
Returns list of full paths. If outfile is set, writes a CSV with columns: gene, file, path.
"""
function create_combined_files(resultfolder::String, filepattern::String, coupling_field::String, G::Tuple, R::Tuple; outfile::String="")
    files = String[]
    ...
    if !isempty(outfile)
        ...
        f = open(outfile, "w")
        writedlm(f, [["file" "path"]], ',')
        for path in files
            writedlm(f, [[basename(path) path]], ',')
        end
        close(f)
    end
    return files
