...
parametes are required to be on the full real domain
"""
...
function metropolis_hastings(data,model,options)
    components = make_components(data,model)
    param,d = initial_proposal(model)
    ll,predictions = loglikelihood(components,param,data,model)
    cyclefrac = .1
    ...
    if options.annealsteps > 0
        param,parml,ll,llml,predictions,temp = anneal(components,predictions,param,parml,ll,llml,d,model.proposal,data,model,options.annealsteps,options.temp,options.tempanneal,time(),maxtime*options.annealsteps/totalsteps)
    end
    if options.warmupsteps > 0
        param,parml,ll,llml,d,proposalcv,predictions = warmup(components,predictions,param,param,ll,ll,d,model.proposal,data,model,options.warmupsteps,options.temp,time(),maxtime*options.warmupsteps/totalsteps)
    end
    fit=sample(components,predictions,param,parml,ll,llml,d,proposalcv,data,model,options.samplesteps,options.temp,time(),maxtime*options.samplesteps/totalsteps)
...
"""
function anneal(components,predictions,param,parml,ll,llml,d,proposalcv,data,model,samplesteps,temp,tempanneal,t1,maxtime)
    ...
    while  step < samplesteps && time() - t1 < maxtime
        step += 1
        _,predictions,param,ll,prior,d = mhstep(components,predictions,param,ll,prior,d,proposalcv,model,data,tempanneal)
        if ll < llml
            llml,parml = ll,param
        end
        parout[:,step] = param
...
"""
function warmup(components,predictions,param,parml,ll,llml,d,proposalcv,data,model,samplesteps,temp,t1,maxtime)
    parout = Array{Float64,2}(undef,length(param),samplesteps)
    prior = logprior(param,model)
    step = 0
    accepttotal = 0
    while  step < samplesteps && time() - t1 < maxtime
        step += 1
        accept,predictions,param,ll,prior,d = mhstep(components,predictions,param,ll,prior,d,proposalcv,model,data,temp)
        if ll < llml
            llml,parml = ll,param
        end
        parout[:,step] = param
    ...
    if isposdef((2.4)^2 / length(param)*covparam) && step > 1000 && accepttotal/step > .1
        d=proposal_dist(param,covparam)
        proposalcv = covparam
    end
...
"""
function sample(components,predictions,param,parml,ll,llml,d,proposalcv,data,model,samplesteps,temp,t1,maxtime)
...
"""
function mhstep(components,predictions,param,ll,prior,d,proposalcv,model,data,temp)
    paramt,dt = proposal(d,proposalcv)
    priort = logprior(paramt,model)
    llt,predictionst = loglikelihood(components,paramt,data,model)
...
proposal_dist(param::Float64,cv::Float64) = Normal(param,cv*abs(param))

function proposal_dist(param::Vector,cv::Float64)
    d = Vector{Normal{Float64}}(undef,0)
    for i in eachindex(param)
        push!(d,Normal(param[i],cv))
    end
...
"""
compute_stats(paramin::Array{Float64,2},model)
...
"""
function loglikelihood(components,param,data,model)
    predictions = likelihoodfn(components,param,data,model)
    hist = datahistogram(data)
...
function deviance(fit::Fit,data::HistogramData,model)
    h=likelihoodfn(make_components(data,model),fit.parml,data,model)
    deviance(h,data)
end

function deviance(data::HistogramData,model::StochasticGRmodel)
    h = likelihoodfn(make_components(data,model),model.rates[model.fittedparam],data,model)
    deviance(h,data)
end
