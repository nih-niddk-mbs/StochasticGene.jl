...
"""
    fit_rna(nchains::Int,gene::String,cell::String,fittedparam::Vector,fixedeffects::Tuple,datacond,G::Int,maxtime::Float64,infolder::String,resultfolder::String,datafolder,fish::Bool,inlabel,label,nsets::Int,cv=0.,transient::Bool=false,samplesteps::Int=100000,warmupsteps=20000,annealsteps=100000,temp=1.,tempanneal=100.,root = "/home/carsonc/scrna/",yieldprior = 0.05,ejectprior = 1.0)
    fit_rna(nchains::Int,data::AbstractRNAData,gene::String,cell::String,fittedparam::Vector,fixedeffects::Tuple,datacond,G::Int,maxtime::Float64,infolder::String,resultfolder::String,datafolder,fish::Bool,inlabel,label,nsets,cv=0.,transient::Bool=false,samplesteps::Int=100000,warmupsteps=20000,annealsteps=100000,temp=1.,tempanneal=100.,root = "/home/carsonc/scrna/",yieldprior = 0.05,ejectprior = 1.0)
...
function fit_rna(nchains::Int,gene::String,cell::String,fittedparam::Vector,fixedeffects::Tuple,transitions::Tuple,datacond,G::Int,maxtime::Float64,infolder::String,resultfolder::String,datafolder,fish::Bool,inlabel,label,nsets::Int,cv=0.,transient::Bool=false,samplesteps::Int=100000,warmupsteps=0,annealsteps=0,temp=1.,tempanneal=100.,root = ".",yieldprior::Float64=0.05,priorcv::Float64=10.,decayrate=-1.)
...
function fit_rna(nchains::Int,data::AbstractRNAData,gene::String,cell::String,fittedparam::Vector,fixedeffects::Tuple,transitions::Tuple,datacond,G::Int,maxtime::Float64,infolder::String,resultfolder::String,datafolder,fish::Bool,inlabel,label,nsets,cv=0.,transient::Bool=false,samplesteps::Int=100000,warmupsteps=0,annealsteps=0,temp=1.,tempanneal=100.,root = ".",yieldprior::Float64=0.05,priorcv::Float64=10.,decayrate=-1.)
    ...
    model = model_rna(gene,cell,G,cv,fittedparam,fixedeffects,transitions,inlabel,infolder,nsets,root,data,yieldprior,decayrate,LogNormal,priorcv,true)
    ...
    try
        ...
        optimized = Optim.optimize(x -> lossfnc(x,data,model),log.(fit.parml),LBFGS())
    catch
...
lossfnc(x,data,model) = loglikelihood(exp.(x),data,model)[1]
...
function model_rna(gene::String,cell::String,G::Int,cv,fittedparam,fixedeffects,transitions,inlabel,infolder,nsets,root,data,yieldprior,decayrate::Float64,fprior=LogNormal,priorcv=10.,verbose=true)
    if decayrate < 0
    ...
    if nsets == 1
        ...
        for i in nsets
            if G == 1
                push!(ejectrate,mean_histogram(data.histRNA[i]) * decayrate/nalleles * yieldprior)
            else
                push!(ejectrate,yieldprior)
            end
        end
        push!(decayrate,decay)
    end
    r = getr(gene,G,nalleles,decayrate,ejectrate,inlabel,infolder,nsets,root,verbose)
    model = model_rna(data,r,G,nalleles,nsets,cv,fittedparam,fixedeffects,transitions,data.nRNA,decayrate,ejectrate,fprior,priorcv)
    return model
end


function model_rna(data,r::Vector,G::Int,nalleles::Int,nsets::Int,propcv,fittedparam,fixedeffects,transitions,nhist,decayprior,ejectprior,f=LogNormal,cv=10.)
    d = prior_rna(r,G,nsets,fittedparam,decayprior,ejectprior,f,cv)
    model_rna(data,r::Vector,d,G::Int,nalleles,propcv,fittedparam,fixedeffects,transitions,nhist)
end
function model_rna(data,r::Vector,d,G::Int,nalleles,propcv,fittedparam,fixedeffects,transitions,nhist,method=0)
...
"""
transient_rna(nchains,gene::String,fittedparam,cond::Vector,G::Int,maxtime::Float64,infolder::String,resultfolder,datafolder,inlabel,label,nsets,samplesteps::Int=40000,warmupsteps=20000,annealsteps=100000,temp=1.,tempanneal=100.,root = "/home/carsonc/scrna/")

make structures for transient model fit
"""
function transient_rna(nchains,gene::String,cell,fittedparam,cond::Vector,G::Int,maxtime::Float64,infolder::String,resultfolder,datafolder,inlabel,label,nsets,samplesteps::Int=40000,warmupsteps=20000,annealsteps=100000,temp=1.,tempanneal=100.,root = "/home/carsonc/scrna/")
...
function setrate(G::Int,nsets::Int,decayrate::Vector,ejectrate::Vector,cv::Float64 = 10.)
    rm = Vector{Float64}(undef,0)
    rc = similar(rm)
    for i in 1:nsets
        rm = vcat(rm,[repeat([0.01;0.1],(G-1));ejectrate[i];decayrate[i]])
        rc = vcat(rc,[cv*ones(2*(G-1));cv;0.01])
    end
    return rm,rc
end
