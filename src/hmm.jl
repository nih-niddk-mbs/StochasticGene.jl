...
TBW
"""
function ll_background(obs::Float64, d::Vector{Distribution{Univariate,Continuous}}, a::Matrix, p0, Nstate, nframes, weight)
    _, C = forward(a::Matrix, set_b_background(obs, d), p0, Nstate, nframes)
    weight * sum(log.(C))
end
...
function ll_hmm_hierarchical(rshared, rindividual::Matrix, Nstate, components::TRGComponents, n_noiseparams::Int, reporters_per_state, probfn, interval, trace)
    ...
    lb = trace[3] > 0 ? length(trace[1]) * ll_background(rshared[end-n_noiseparams+1, 1], d, a, p0, Nstate, trace[4], trace[3]) : 0.0
