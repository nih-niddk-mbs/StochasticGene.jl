...
"""
function write_traces(folder, datapath, datacond, interval, ratetype::String="median", start=1, stop=-1, probfn=prob_Gaussian, noiseparams=4, weightind=0, splicetype=""; label = "", state=false, hierarchical=false)
    for (root, dirs, files) in walkdir(folder)
        for f in files
            if occursin("rates", f) && occursin(datacond, f) && occursin(label, f)
                parts = fields(f)
                G, R, S, insertstep = decompose_model(parts.model)
                r = readrates(joinpath(root, f), get_row(ratetype))
                out = joinpath(root, replace(f, "rates" => "predictedtraces", ".txt" => ".csv"))
