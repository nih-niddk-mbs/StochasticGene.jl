...
- **Cluster / Biowulf / batch fits:** read [Cluster and batch workflows](cluster_batch_workflows.md) for `makeswarm`, `makeswarm_genes`, staged run specs, and the **coupled-model workflow** (fit units separately, merge rates with `create_combined_file`, then run the coupled fit).
