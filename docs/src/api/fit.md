...
- **`datatype`**: String or symbol for a single legacy datatype, e.g. `"rna"`, `:trace`, `"rnadwelltime"`, `"tracejoint"`. A tuple or vector such as `(:rna, :dwelltime)` requests the [`CombinedData`](combined_data.md) path.
